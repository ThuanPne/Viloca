...
import {
  ...
  Modal, TextInput, ActivityIndicator, Animated, Pressable,
} from 'react-native';
...
export default function ExperienceDetailScreen() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .single()
      .then(({ data }) => {
        setLocation(data as Location | null);
        setLoadingLoc(false);
      });
  }, [id]);
  ...
  if (!location) {
    return (
      <View style={styles.center}>
        <Text style={{ color: colors.textMuted }}>Không tìm thấy địa điểm</Text>
      </View>
    );
  }

  const coverUri    = location.cover_image ?? `https://picsum.photos/seed/${location.id}/800/600`;
  const durationHrs = (location.duration_minutes / 60).toFixed(1);
  const displayLoc  = [location.district, location.city].filter(Boolean).join(', ');
  const desc        = location.short_description ?? location.description ?? '';
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={styles.infoSection}>
          <View style={styles.categoryRow}>
            <Badge label={location.category} color="primary" />
          ...
          {displayLoc ? <Text style={styles.location}>📍 {displayLoc}</Text> : null}

          <View style={styles.metaRow}>
            <View style={styles.metaItem}>
              <Ionicons name="time-outline" size={16} color={colors.textMuted} />
              <Text style={styles.metaText}>{durationHrs} giờ</Text>
            </View>
            <View style={styles.metaSep} />
            <View style={styles.metaItem}>
              <Ionicons name="cash-outline" size={16} color={colors.textMuted} />
              <Text style={styles.metaText}>{formatPrice(location.price_per_person)} / người</Text>
            </View>
          </View>
        </View>
        ...
        {location.opening_hours ? (
          <View style={styles.infoBox}>
            ...
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoText}>{location.opening_hours}</Text>
            </View>
          </View>
        ) : null}
          ...
          <View style={styles.section}>
            ...
            <View style={styles.tagsWrap}>
              {location.vibes.map((vibe) => (
                <Tag key={vibe} label={vibe} color="forest" />
              ))}
            </View>
          </View>
        )}
      ...
      <View style={styles.bottomBar}>
        <View>
          ...
          <Text style={styles.priceValue}>{formatPrice(location.price_per_person)}</Text>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <View style={styles.expMini}>
              <Image source={{ uri: coverUri }} style={styles.expMiniImg} />
              <View style={{ flex: 1 }}>
                <Text style={styles.expMiniTitle} numberOfLines={1}>{location.name}</Text>
                {displayLoc ? <Text style={styles.expMiniLoc}>📍 {displayLoc}</Text> : null}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  infoBox:            { marginHorizontal: spacing.lg, marginTop: spacing.md, backgroundColor: colors.bgCard, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, padding: spacing.md },
